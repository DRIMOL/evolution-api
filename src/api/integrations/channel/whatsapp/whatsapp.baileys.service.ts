          ...
          if (received.message?.pollUpdateMessage) {
            ...
            try {
              ...
              if (pollCreationMessageFull && 'message' in pollCreationMessageFull && pollCreationMessageFull.message?.pollCreationMessageV3 && pollCreationMessageFull.message?.messageContextInfo) {
                ...
                this.logger.info(`[PollVoteDebug] messageSecret encontrado: ${messageSecret ? 'Sim' : 'Não'}`);

                if (!messageSecret) {
                  this.logger.warn(`[PollVoteDebug] Message secret não encontrado para a enquete ${pollMsgId}, não é possível descriptografar o voto.`);
                  continue;
                }

                // Log adicionado para verificar messageSecret
                this.logger.info(`[PollVoteDebug] Verificando messageSecret antes de descriptografar. Tipo: ${typeof messageSecret}, Comprimento: ${messageSecret?.length}, Valor (início): ${typeof messageSecret === 'string' ? messageSecret?.slice(0, 10) : Buffer.isBuffer(messageSecret) ? messageSecret.toString('base64').slice(0,10) : 'N/A'}...`);
