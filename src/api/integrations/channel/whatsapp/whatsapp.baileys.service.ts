          ...
          if (received.message?.pollUpdateMessage) {
            ...
            try {
              ...
              if (pollCreationMessageFull && 'message' in pollCreationMessageFull && pollCreationMessageFull.message?.pollCreationMessageV3 && pollCreationMessageFull.message?.messageContextInfo) {
                ...
                // Log adicionado para verificar messageSecret (corrigido para TS)
                this.logger.info(`[PollVoteDebug] Verificando messageSecret antes de descriptografar. Tipo: ${typeof messageSecret}, Comprimento: ${messageSecret?.length}, Valor (início): ${Buffer.isBuffer(messageSecret) ? messageSecret.toString("base64").slice(0,10) : "Não é Buffer"}...`);
