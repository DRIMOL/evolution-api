...
import makeWASocket, {
  ...
  WAPresence,
  WASocket,
  decryptPollVote, // <<< Import decryptPollVote
} from 'baileys';
      ...
      try {
        for (const received of messages) {
          ...
          if (
            ...
            // received.message?.pollUpdateMessage || // Removed to allow processing poll updates within this handler
            !received?.message
          ) {
            continue;
          }
          ...
          // --- INÍCIO: Adição para Tratamento e Descriptografia de Atualização de Enquete (com Logs Detalhados) ---
          if (received.message?.pollUpdateMessage) {
            this.logger.info(`[PollVoteDebug] Recebida atualização de enquete para a chave: ${JSON.stringify(received.message.pollUpdateMessage.pollCreationMessageKey)}`);
            try {
              // Busca a mensagem original de criação da enquete.
              const pollCreationKey = received.message.pollUpdateMessage.pollCreationMessageKey;
              this.logger.info(`[PollVoteDebug] Buscando mensagem original com a chave: ${JSON.stringify(pollCreationKey)}`);
              const pollCreationMessageFull = await this.getMessage(pollCreationKey, true); // Usando 'true' para buscar a mensagem completa
              this.logger.info(`[PollVoteDebug] Mensagem original da enquete encontrada: ${pollCreationMessageFull ? 'Sim' : 'Não'}`);
              // Log detalhado do objeto retornado por getMessage
              ...
              if (pollCreationMessageFull && 'message' in pollCreationMessageFull && pollCreationMessageFull.message?.pollCreationMessageV3 && pollCreationMessageFull.message?.messageContextInfo) {
                const pollCreationMessage = pollCreationMessageFull.message.pollCreationMessageV3; // Corrigido para V3
                          const messageSecretString = pollCreationMessageFull.message.messageContextInfo.messageSecret;
                const pollCreatorJid = pollCreationMessageFull.key.remoteJid;
                const pollMsgId = pollCreationMessageFull.key.id;
                const voterJid = received.key.participant || received.key.remoteJid;

                this.logger.info(`[PollVoteDebug] pollCreatorJid: ${pollCreatorJid}, pollMsgId: ${pollMsgId}, voterJid: ${voterJid}`);
                this.logger.info(`[PollVoteDebug] messageSecretString encontrado: Sim`);

                if (!messageSecretString || typeof messageSecretString !== 'string') {
                  this.logger.warn(`[PollVoteDebug] messageSecret não é uma string válida ou não foi encontrado para a enquete ${pollMsgId}, não é possível descriptografar o voto.`);
                  continue;
                }

                // Converte messageSecret (string base64) para Buffer
                const messageSecretBuffer = Buffer.from(messageSecretString, 'base64');
                this.logger.info(`[PollVoteDebug] Verificando messageSecretBuffer antes de descriptografar. Tipo: ${typeof messageSecretBuffer}, Comprimento: ${messageSecretBuffer?.length}, É Buffer: ${Buffer.isBuffer(messageSecretBuffer)}`);
                ...
                this.logger.info(`[PollVoteDebug] Resultado de decryptPollVote (hashes): ${JSON.stringify(decryptedVotes.selectedOptions)}`);

                // Mapeia os hashes descriptografados para os nomes das opções originais
                const selectedOptionNames = [];
                this.logger.info(`[PollVoteDebug] Iniciando mapeamento de hashes para nomes. Opções originais: ${JSON.stringify(pollCreationMessage.options.map(o => o.optionName))}`);
                for (const decryptedHash of decryptedVotes.selectedOptions) {
                  const hashHex = Buffer.from(decryptedHash).toString('hex').toUpperCase();
                  this.logger.info(`[PollVoteDebug] Tentando mapear hash descriptografado: ${hashHex}`);
                  let foundMatch = false;
                  for (const option of pollCreationMessage.options) {
                    // Calcula o hash SHA-256 do nome da opção original
                    const optionHash = Buffer.from(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(option.optionName)))
                                         .toString('hex')
                                         .toUpperCase();
                    this.logger.info(`[PollVoteDebug] Comparando com hash da opção '${option.optionName}': ${optionHash}`);
                    if (optionHash === hashHex) {
                      this.logger.info(`[PollVoteDebug] Hash correspondente encontrado para a opção: ${option.optionName}`);
                      selectedOptionNames.push(option.optionName);
                      foundMatch = true;
                      break; // Encontrou a opção correspondente
                    }
                  }
                  if (!foundMatch) {
                     this.logger.warn(`[PollVoteDebug] Nenhum hash correspondente encontrado para o hash descriptografado: ${hashHex}`);
                  }
                }

                this.logger.info(`[PollVoteDebug] Nomes finais das opções selecionadas: ${JSON.stringify(selectedOptionNames)}`);

                // Prepara os dados para enviar via webhook ou evento interno
                const payload = {
                  event: Events.POLL_UPDATE,
                  instance: this.instance.name,
                  pollCreationKey: received.message.pollUpdateMessage.pollCreationMessageKey,
                  voterJid: voterJid,
                  selectedOptions: selectedOptionNames, // Votos descriptografados
                  timestamp: received.message.pollUpdateMessage.senderTimestampMs,
                };

                this.logger.info(`[PollVoteDebug] Enviando payload para webhook: ${JSON.stringify(payload)}`);
                // Envia os dados via Webhook e/ou EventEmitter
                this.sendDataWebhook(Events.POLL_UPDATE, payload);
                // this.eventEmitter.emit(Events.POLL_UPDATE, payload); // Descomente se precisar emitir internamente

              } else {
                this.logger.warn(`[PollVoteDebug] Não foi possível encontrar a mensagem original de criação da enquete, o contexto ou a estrutura esperada para a chave: ${JSON.stringify(received.message.pollUpdateMessage.pollCreationMessageKey)}. pollCreationMessageFull: ${!!pollCreationMessageFull}, hasMessageProp: ${pollCreationMessageFull ? ('message' in pollCreationMessageFull) : 'N/A'}, hasPollCreation: ${pollCreationMessageFull && 'message' in pollCreationMessageFull ? !!pollCreationMessageFull.message?.pollCreationMessage : 'N/A'}, hasContextInfo: ${pollCreationMessageFull && 'message' in pollCreationMessageFull ? !!pollCreationMessageFull.message?.messageContextInfo : 'N/A'}`);
              }
            } catch (error) {
              this.logger.error(`[PollVoteDebug] Erro ao processar e descriptografar atualização de enquete: ${error?.message || error}`);
              this.logger.error(`[PollVoteDebug] Stack trace: ${error?.stack}`);
            }
            // Como esta mensagem é APENAS uma atualização de enquete, pulamos o resto do processamento
            continue; 
          }
          // --- FIM: Adição para Tratamento e Descriptografia de Atualização de Enquete ---
